...
export default function AgentList() {
  const { agents, fetchAgent, isLoading } = useAgentStore();
  ...
  const PagenationImg = require("@/assets/images/pagenationImg.png");
  ...
  useEffect(() => {
    if (agents.length === 0) {
      fetchAgent(true).catch((error) => {
        console.error("Failed to fetch agents:", error);
        Alert.alert("Error", "Failed to load agents");
      });
    }
  }, [agents.length, fetchAgent]);
  ...
  return (
    ...
    >
      ...
      <Image source={PagenationImg} style={{alignSelf:"center", marginVertical:10}}/>
