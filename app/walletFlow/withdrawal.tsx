...
export default function BankAccountsScreen() {
  ...
  const handleSelect = (acct: typeof accounts[0]) => {
    router.push({
      pathname: "/walletFlow/withdrawAmount",
      params: {
        accountId: acct.id,
    ...
    return (
      ...
      >
        <XStack justifyContent="space-between" alignItems="center">
          <TouchableOpacity activeOpacity={0.7} onPress={() => handleSelect(acct)} style={{ flex: 1 }}>
            <YStack>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <YStack flex={1} backgroundColor="#fff">
        <YStack padding="$4">
          <ScreenHeader title="Select bank account" onBackPress={() => router.back()} />
        </YStack>
...
const styles = StyleSheet.create({
  ...
  accountNumber: { color: "#888", marginTop: 2, fontSize: 18, letterSpacing: 1 },
