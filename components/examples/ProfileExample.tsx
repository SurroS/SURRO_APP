...
export const ProfileExample = () => {
    ...
    return (
        <YStack padding="$4" gap="$3">
            <Text fontSize="$6" fontWeight="bold">Profile Management</Text>
            ...
            <Button
                onPress={handleUpdateMedicalProfile}
                disabled={isLoading}
            >
            ...
            {surrogateProfile && (
                <YStack gap="$2">
                    <Text fontSize="$4" fontWeight="bold">Current Profile</Text>
                    <Text>Name: {surrogateProfile.firstName} {surrogateProfile.lastName}</Text>
            ...
            {medicalProfile && (
                <YStack gap="$2">
                    <Text fontSize="$4" fontWeight="bold">Medical Profile</Text>
                    <Text>Genotype: {medicalProfile.genotype}</Text>
                    <Text>Blood Group: {medicalProfile.bloodGroup}</Text>
                    <Text>Pregnancy Experience: {medicalProfile.pregnancyExperience ? 'Yes' : 'No'}</Text>
                    <Text>Number of Children: {medicalProfile.numberofChildren}</Text>
                    <Text>Caesarean Section: {medicalProfile.ceasareanSection ? 'Yes' : 'No'}</Text>
                    <Text>Chronic Illness: {medicalProfile.chronicIllnessDetails}</Text>
                    <Text>Pregnancy Complications: {medicalProfile.pregnancyComplicationsDetails}</Text>
                </YStack>
            )}
        </YStack>
    );
